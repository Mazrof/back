...
describe('channelMemberService', () => {
  ...
  describe('checkChannelMemberPermission', () => {
    ...
    it('should not throw an error if the user is an admin', async () => {
      ...
      await expect(
        Service.checkChannelMemberPermission(1, 1)
  ...
  //     // Mock `findChannel` to simulate finding a channel
  //     jest.spyOn(Service, 'findChannel').mockResolvedValue(undefined); // Ensure it returns undefined as no channel is found
  //
  //     // Mock `findExistingMember` to simulate no member found (return null)
  //     (Repository.findExistingMember as jest.Mock).mockResolvedValue(null); // Simulating no member
  //
  //     // Expect `checkChannelMember` to throw an error if no member is found
  //     await expect(Service.checkChannelMember(1, 1)).rejects.toThrowError(
  //       new AppError('User is not a member of the channel', 403)
  //     );
  //
  //     // Ensure `findExistingMember` is called with the correct arguments
  ...
  describe('updateChannelMember', () => {
    it('should throw an error if no data is provided to update', async () => {
      await expect(Service.updateChannelMember(1, 1, 1, {})).rejects.toThrow(
    ...
    it('should throw an error if the member is not found or inactive', async () => {
      ...
      await expect(
        Service.updateChannelMember(1, 1, 1, { role: 'admin' })
      ).rejects.toThrow(new AppError('Not Authorized', 403));
    });
    ...
    it('should update the channel member successfully if data is valid', async () => {
      ...
      // Mock the checkAdmin function to avoid authorization issues
      jest.spyOn(Service, 'checkAdmin').mockResolvedValue(undefined);

      const result = await Service.updateChannelMember(1, 1, 1, {
        role: 'admin',
      });
